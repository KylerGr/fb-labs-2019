...
def generate():
	...
	if p * q >= p1 * q1:
		...
		print("The p value for the first user is: " + str(p))
		print("The q value for the first user is: " + str(q))
		print("----------------------------------------------")
		print("The p value for the second user is: " + str(p1))
		print("The q value for the second user is: " + str(q1))
	return p, q, p1, q1

def open_key(p, q, p1, q1):
	f = (p - 1) * (q - 1)
	f1 = (p1 - 1) * (q1 - 1)
	for i in range(0, f):
		e = randrange(2, f - 1)
...
def cipher_message(p, q, p1, q1, e, M):
	C = pow(M, e, int(p * q))
	print("Ciphered message for the user: " + str(C))
	return C


def decipher_message(p, q, p1, q1, d, C):
	m = pow(C, d, int(p * q))
	print("Deciphered message for the first user is: " + str(m))
	return m

def signature(p, q, p1, q1, d, M):
	S = pow(M, d, int(p * q))
	print("Signature for the user: " + str(S))
	return S

def check_signature(p, q, p1, q1, e, S, M):
	R = pow(S, e, int(p * q))
	if R == M:
		return "Successfully checked!"

def key_to_send(p, q, p1, q1, e, d):
	k = randrange(0, p * q)
	k1 = pow(k, e, p1 * q1)
	s = pow(k, d, p * q)
	s1 = pow(s, e, p1 * q1)
...
def receive_key(k1, s1, p, q, p1, q1, e, d):
	k = pow(k1, d, p1 * q1)
	s = pow(s1, d, p1 * q1)
	c = pow(s, e, p * q)
